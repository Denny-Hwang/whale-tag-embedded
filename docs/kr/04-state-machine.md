...
## 7. `tmp/state_machine.{c,h}` 에 대하여 (제거됨)

저장소 루트의 `tmp/`에는 상태 머신의 **오래된 작업 사본**이 커밋되어 있었습니다
(PREDEPLOY 상태가 없고, FLOAT_DETECTION이 켜져 있는 등 구버전). 빌드에 포함되지 않는
잔재물이라 정리 PR에서 삭제되었습니다. 흥미롭게도 5절의 "조건 반전" 버그는 이
구버전에는 없었습니다 — 리팩터링 과정에서 회귀했던 것으로, 현재는 앱 코드에서
수정되었습니다.
